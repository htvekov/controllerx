...
ESPHome is at max with all these sensors, some schedulers running and an extensive display lambda. Addding just one extra sensor will make ESPHome crash on boot. Omitting  `fast_connect: true`from WiFi configuration in YAML will also send ESPHome into an eternal stack trace error boot loop 💀
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTIxNTI4OTUzOSwxNDc0NDY2Mjc0LC02ND
